package com.neeko.section06.tree;

import java.util.ArrayList;
import java.util.List;

/* 이진 트리
* 각 노드가 최대 두 개의 자식을 가지며 왼쪽 자식과 오른쪽 자식으로 구분된다.
*
* 이진 탐색 트리
* 이진 트리의 구조를 가지면서 추가로 왼쪽 서브트리의 모든 노드는 현재 노드 값보다 작고,
* 오른쪽 서브 트리의 모든 노드는 현재 노드 값보다는 크다는 규칙(정렬 속성)을 만족한다.
*
* => 제한된 자식의 수와 정렬 속성 덕분에 트리 순회, 탐색, 삽입, 삭제 등의 알고리즘을
...
public class BinarySearchTree<T extends  Comparable<T>> {
    ...
    private void preOrderRec(Node<T> node, List<T> result) {
        if(node !=null){        // 재귀 종료 조건 (삽입한 데이터가 없음)
    ...
    private Node<T> deleteRec(Node<T> node, T data) {
        ...
        if(data.compareTo(node.data)<0){
            ...
            if(node.left ==null)
                ...
                return node.left;
            // 두 자식이 있는 경우 : 오른쪽 서브트리의 최소 값을 찾아오고 기존 위치에서는 삭제
