...
public class G_Knapsack {
    public static int solution(String input) throws IOException {
        ...
        for(int i = 1; i <= n; i++) {
            ...
            // 가방의 키로수까지 반복
            for(int j = 1; j <= k; j++) {
                // 1. 물건이 가방에 들어갈 수 있는지 확인

                    if(j < w){
                        // 1-1. 들어갈 수 없다면? 가치값의 변화가 없으므로 이전 행의 값을 그대로 반영(배열의 어떤 값을 반영해야 할 지 생각)
                              dp[i][j] = dp[i-1][j];
                    }else{
                        // 1-2. 들어갈 수 있다면? (담는게 가치가 높을 지 안 담는 게 가치가 높을 지)
                        // 담는다 : 물건 가치 + 해당 무게 빼고 남은 가치
